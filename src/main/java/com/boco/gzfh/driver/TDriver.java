...
import cms.impl.GroupGzfhCmsProtocol;
import com.boco.gzfh.constant.ConstVarient;
import com.boco.gzfh.constant.DriverPubConst;
import com.boco.gzfh.tcpcomm.IMessageListener;
import com.boco.gzfh.tcpcomm.IPortStatusListener;
import com.boco.gzfh.tcpcomm.TTCPComm;
...
import com.boco.gzfh.tcpcomm.RequestEncoder;
import com.boco.gzfh.tcpcomm.ResponseDecoder;
import com.boco.gzfh.constant.DriverConst;
import com.boco.gzfh.tcpcomm.MsgDealHandler;
import com.boco.gzfh.model.Request;
...
public class TDriver implements IMessageListener, IPortStatusListener {
    ...
    public boolean sendData(String businessNo, DevVarInfo devVarInfo)
    {
        ...
        try {
			switch (sendDriverVarInfo.getDwSendCmd()) {
				case DriverConst.Const_Cmd_PlayLst: //播放表
					...
					GroupGzfhCmsProtocol CmsProtocol = new GroupGzfhCmsProtocol();
				...
				case DriverConst.Const_Cmd_ReadCms: //读取屏幕播放表信息
					break;
				case DriverConst.Const_Cmd_BrightMode: //设备屏幕亮度模式
					try {
						//0-自动；1-手动
						...
						LOGGER.error("控制情报板亮度时，下发的数据无法转换为整型数.错误信息:" + ex.getMessage());
						return result;
					}
					break;
				case DriverConst.Const_Cmd_Bright: //设备屏幕亮度
					try {
						fValue = Integer.parseInt(sendStr);
						Dummy = CoderUtils.IntTo2SizeString(fValue) + CoderUtils.IntTo2SizeString(fValue) + CoderUtils.IntTo2SizeString(fValue);
						ArrSend = Dummy.getBytes("GBK");
					} catch (Exception ex) {
						LOGGER.error("控制情报板亮度时，下发的数据无法转换为整型数.错误信息:" + ex.getMessage());
						return result;
        ...
        Frm = new Request(this.faddress, sendDriverVarInfo.getDwSendCmd(), ArrSend);
		...
		try {
			...
			switch (Cmd) {
				...
				case DriverConst.Const_Cmd_ScrOnOff:
					...
					break;
				//调节亮度模式
				case DriverConst.Const_Cmd_BrightMode:
					if (resp.getDataArray().length == 4 && resp.getDataArray()[0] == DriverConst.Const_Answer_Success) {
						//发送成功
						RabbitmqCtrlCmdback(businessNo, true);
					} else {
						//发送失败
	...
	public void RabbitmqCtrlCmdback(String businessNo,boolean SendFlag) {
		try {
			...
			Identitytemp.setSourceId("collctrsvr_gzfh_cms");
			...
			for (TDriverVarInfo driverVarInfo : this.devDriverVarInfo) {
				...
				if (driverVarInfo.getSzFuncAbbr().indexOf("ER") >= 0) {
					try {
						...
						int ERValue = Data[0] * 256 + Data[1];
						ERValue = ERValue & (1 << bit);
						if (ERValue > 0)
							ERValue = 1;
						else
							ERValue = 0;
